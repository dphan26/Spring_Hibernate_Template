...
public class CacheHelper implements InitializingBean{
	...
	public Object getValueFromCache(String cacheName, String valueKey){
		...
		if (cache.isKeyInCache(valueKey)) {
			...
			if (cacheElement == null) {
				if (cache.isKeyInCache(valueKey)) {
					/**
					 * This element is not an expired element (if it's expired - due to timeToLiveSeconds config, 
					 * the call cache.get(valueKey) above will remove it from the cache)
					 * => we log out error to warn users
					 */
			...
			if(DebugConstants.DEBUG_CACHE_DETAILS){
				System.out.println("Cached data found for [" + cacheName + "=>" + valueKey + "=" + result + "]'. Total get-from-cache time is " + (System.currentTimeMillis() - start) + " milli seconds.");
			}
		}
		return result;
	}
	...
	protected List<Object> getValuesFromCache(String cacheName) {
		...
		if (keysList != null && ! keysList.isEmpty()) {
			...
			for (String valueKey : keysList) {
				...
				if(DebugConstants.DEBUG_CACHE_DETAILS){
					System.out.println("Cached data found for [" + cacheName + "=>" + valueKey + "=" + cacheValue + "]'. Total get-from-cache time is " + (System.currentTimeMillis() - start) + " milli seconds.");
				}
			}
		}
		return result;
	...
	public static void debugSecondLevelCache(Session session){
		if(!DebugConstants.DEBUG_SECOND_LEVEL_CACHE_DETAILS){
			return;
		}
	...
	public static void debugCacheManager(CacheManager cacheManager){
		if(!DebugConstants.DEBUG_CACHE_MANAGER){
			return;
		}
	...
	public static void clearCacheFromCacheGlobal(CacheManager cacheManager, String keyName) {
		Cache cache = cacheManager.getCache(CacheConstants.SiteGlobal.SITE_GLOBAL_CACHE);
		if (cache == null) {
			return;
		}
